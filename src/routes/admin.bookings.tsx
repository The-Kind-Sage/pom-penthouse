...
function BookingsPage() {
  ...
  const isLoading = (tab === "bookings" && loadingBookings) || (tab === "inquiries" && loadingInquiries);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-foreground/60">Manage all reservations & inquiries</p>
        </div>
        {tab === "bookings" && (
      ...
      {tab === "bookings" ? (
        ...
        <>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-1.5 text-sm flex-1 min-w-[200px] max-w-sm">
              <Search size={16} className="text-foreground/40" />
              <input placeholder="Search by name or apartment..." value={search} onChange={(e) => setSearch(e.target.value)} className="bg-transparent outline-none w-full" />
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="text-left px-4 py-3 font-medium">Guest</th>
                    <th className="text-left px-4 py-3 font-medium">Apartment / Room</th>
                    <th className="text-left px-4 py-3 font-medium">Check-in</th>
                    <th className="text-left px-4 py-3 font-medium">Check-out</th>
                  ...
                  {inquiries.filter((i) => {
                    ...
                    <tr key={i.id} className="hover:bg-muted/30 transition">
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-foreground/60">{i.email}</p>
                      </td>
                      <td className="px-4 py-3">{i.apartment}</td>
                      <td className="px-4 py-3">{i.checkin}</td>
                      <td className="px-4 py-3">{i.checkout}</td>
          ...
          {selectedInquiry && (
            <>
              <div className="fixed inset-0 z-50 bg-black/40" onClick={() => setSelectedInquiry(null)} />
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div className="bg-background border rounded-2xl p-6 max-w-md w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between"><span className="text-foreground/60">Apartment</span><span>{selectedInquiry.apartment}</span></div>
                    <div className="flex justify-between"><span className="text-foreground/60">Check-in</span><span>{selectedInquiry.checkin}</span></div>
                    <div className="flex justify-between"><span className="text-foreground/60">Check-out</span><span>{selectedInquiry.checkout}</span></div>
                    <div className="flex justify-between"><span className="text-foreground/60">Guests</span><span>{selectedInquiry.guests}</span></div>
                    ...
                    <div className="border-t pt-2 mt-2 text-xs text-foreground/40">
