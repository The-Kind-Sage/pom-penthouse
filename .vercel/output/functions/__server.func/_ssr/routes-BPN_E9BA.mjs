...
function BookingModal() {
	...
	const submit = (e) => {
		...
		const window = room === "long" ? "your long stay" : `${range.from.toDateString()} → ${range.to.toDateString()}`;
		toast.success(`Booking request sent! We'll confirm availability for ${window} within 2 hours.`, { duration: 6e3 });
		ui.closeBooking();
		resetAll();
	};
